...
def test_invoice_sensor_attributes(mock_coordinator):
    ...
    # Reservation counts
    assert attributes["total_reservations"] == 7
    assert attributes["washer_reservations"] == 4
    assert attributes["dryer_reservations"] == 3        # Due date information
    assert "due_date" in attributes
    assert "due_date_raw" in attributes
    # The exact number might vary slightly depending on time calculation
    assert 4 <= attributes["days_until_due"] <= 5  # Should be approximately 5 days
